...
aerospace focus [-h|--help] [--ignore-floating] [--wrap-around]
                [--boundaries <boundary>] [--boundaries-action <action>]
                (tab-next|tab-prev)
aerospace focus [-h|--help] --tab-index <tab-index>
...
--tab-index <tab-index>::
Focus tab by its 1-based index inside the current accordion tab group.
...
(tab-next|tab-prev)::
Set focus to the next or previous tab inside the current accordion tab group.
In this mode, `--boundaries` must be `workspace` (the default) and `--boundaries-action` can be set to one of `(stop|fail|wrap-around-the-workspace)`.
At tab-group boundaries, `wrap-around-the-workspace` wraps within the current tab group.
