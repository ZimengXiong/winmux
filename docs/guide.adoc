...
* `mode.*.binding-tap`. It falls back to the empty TOML table. Use it for side-specific modifier keys tapped alone (for example `left-alt` or `right-cmd`).
...
You can also bind side-specific modifiers when they are tapped alone:

[source,toml]
----
[mode.main.binding-tap]
    left-alt = 'exec-and-forget open -na "Google Chrome"'
    right-cmd = 'exec-and-forget open -na "Safari"'
----
...
If you want a layout-orientation-independent binding for browser-style tab strips, use `focus (tab-next|tab-prev)`.
